...
Conditioning is inferred from the grouping columns present in
`new_data`: a `site` (and optionally `year`) column with known levels is
conditioned on; factors with no column are handled by `new_levels`. With
`new_data = NULL` the observed data is used and conditioned on its site
and year, so the central estimate agrees with
[`augment()`](https://generics.r-lib.org/reference/augment.html).

## See also

[`kb_predict_weight()`](https://hakaiinstitute.github.io/kelpbio/reference/kb_predict_weight.md),
which summarises these draws.
