...
test_that("a site column conditions on that site", {
  ...
  m_site <- posterior_epred(weight_fit, new_data = nd, new_levels = "average")
  m_bare <- posterior_epred(weight_fit, new_data = data.frame(diameter = c(30, 30)), new_levels = "average")
...
test_that("an unknown site level is sampled, not errored", {
  ...
  m <- posterior_epred(weight_fit, new_data = nd, new_levels = "sample")
...
test_that("representative_site makes a new site borrow a known site's main effects", {
  ...
  rep <- posterior_epred(
    weight_fit,
    new_data = data.frame(diameter = diameter, site = "brand_new_site"),
    new_levels = "average", representative_site = site1
  )
  known <- posterior_epred(
    weight_fit,
    new_data = data.frame(diameter = diameter, site = site1),
    new_levels = "average"
  )
