...
write_fake_chain <- function(path, n_fields = 5L, n_rows = 3L,
                             torn = FALSE, complete = FALSE) {
  header <- paste(c("lp__", paste0("v", seq_len(n_fields - 1L))), collapse = ",")
  ...
  if (torn) {
    lines <- c(lines, paste(as.character(seq_len(n_fields - 2L)), collapse = ","))
  }
  if (complete) {
