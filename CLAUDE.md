...
The analysis project uses `embr`/`cmdstanr`. kelpbio re-implements the
same models using `rstan`/`rstantools` (pre-compiled Stan, no cmdstan
required at runtime). Key analysis scripts:
...
kelpbio works at **site-year resolution — no month dimension** (the
...
- **Backend**: `rstan` + `rstantools`. Stan models live in `inst/stan/`
  and are pre-compiled at `R CMD INSTALL` time.
- **No cmdstan**: users do not need `cmdstanr` or a separate Stan
  installation.
- **Sampling**:
...
1.  **Priors as data** — analysis models hard-code priors; kelpbio
    passes all prior hyperparameters through the `data` block so users
    can adjust them via `kb_priors_*()`.
2.  **Column naming** — analysis uses CamelCase (`SbulbMax`, `PlantW`,
    `Site`, `Year`); kelpbio uses snake_case (`diameter`, `weight`,
    `site`, `year`).
...
- **Testing**: testthat 3e; strict 1:1 test mirroring (`R/<name>.R` ↔︎
  ...
  numerics (test structure + invariants instead). Small pre-built fits
