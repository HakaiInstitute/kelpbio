...
- **(c) Predictions reconstructed R-side from draws.** The in-Stan generated quantities only cover the observed grid; arbitrary `new_data` curves are computed in R from the parameter draws. Per draw the population part is `lp = bWeight30 + bDiameter * log(d/30) + bDiameter2 * log(d/30)^2`. The model has three random effects across two grouping factors — site intercept and site slope (keyed on `site`), and site:year (keyed on `site`, `year`) — so prediction assembles, per draw, the site-intercept, site-slope (multiplies `log(d/30)`), and site:year contributions according to `by`/`uncertainty`:
  - a factor named in `by` uses the observed estimated effect (index the stored `bSite`/`bSiteDiameter`/`bSiteYear` draws);
  - a factor omitted under `marginal` is drawn from its hyperprior (`rnorm(0, sSite)`, `rnorm(0, sSiteDiameter)`, `rnorm(0, sSiteYear)`, mirroring the Stan `normal_rng`);
  - a factor omitted under `typical` is zeroed.
  So `by = NULL` marginal draws all three; `by = "site"` marginal holds the site intercept+slope and draws site:year; `by = c("site","year")` conditions on everything. Enforce: `marginal` requires `by` to omit a random-effect factor, so `by = c("site","year")` under `marginal` errors; `by = "year"` is rejected (no year main effect — year enters only via `site:year`), leaving the valid set `NULL` / `"site"` / `c("site","year")`. `kb_predict_weight_samples()` returns the `[draws × grid]` (× group) array as a draws container; `kb_predict_weight()` is the summariser over it (median + conf_level interval, rounded to `sig_fig`) — one codepath.
