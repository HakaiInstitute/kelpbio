...
- [x] 7.6 Rename the data columns to carry units: `diameter` -> `diameter_mm`, `weight` -> `weight_kg` (the model centres diameter at 30 mm, so the unit is part of the contract). Threads through `kb_check_data_weight`, `assemble_stan_data` (Stan data-block names stay `diameter`/`weight`, so no recompile), `.weight_linpred`/grid builders, the `kb_predict_weight_by(diameter_mm = )` argument, prediction predictor metadata, the `kb_plot_predictions` axis-label map, and the `summary` formula. Rebuild `data_weight_hakai`/`data_weight_sim`/`sim_weight` from their (seeded/deterministic) scripts and migrate the pre-fit `fit_weight`/`weight_fit` objects in place (rename `$data` columns). Update `openspec/specs/data` + `predictions` (and this change's deltas), all affected tests, the demos, and regenerate the `kb_check_data_weight` snapshot. The prior coefficient name `diameter` and the Stan model name `weight` are unchanged.
