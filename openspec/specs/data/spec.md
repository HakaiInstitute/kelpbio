...
The package SHALL also ship `data_weight_sim_nereo`, a small simulated dataset with the same columns (`diameter`, `weight`, `site`, `year`), generated reproducibly from a fixed seed. It is intended for fast tests and runnable examples, not for inference.

The package SHALL also ship `fit_weight_hakai_nereo`, a slim pre-fit `kb_fit_weight` object fitted to `data_weight_sim_nereo` with a reduced number of chains and draws. It is intended for runnable examples and tests, not for inference.

#### Scenario: Hakai dataset is available and valid
- **WHEN** `data_weight_hakai_nereo` is loaded
