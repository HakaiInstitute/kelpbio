...
- **THEN** it returns a tibble with columns `term`, `estimate`, `lower`, `upper`, one row per population-level term (`bWeight`, `bLogPower`, `bFloor`, `bNu`) and per random-effect SD (`sSite`, `sYear`, `sSitePower`, `sSiteYear`, `sWeight`), with the per-level group deviations (`bSite[.]`, `bYear[.]`, `bSitePower[.]`, `bSiteYear[.,.]`) omitted because `include_random_effects` defaults to `FALSE`
