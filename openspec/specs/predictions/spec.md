...
- **THEN** it returns a `D x N` matrix of strictly positive weights drawn from `gamma(shape, shape / eWeight)`; with `new_data = NULL` it returns the stored `yrep`
